...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/messiey/lele/master/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/messiey/lele/master/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/messiey/lele/master/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
dircreate
        ...
        echo -ne "Username admin panel use '@' [Ex: @wongedan_kuwibebas] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@wongedan_kuwibebas"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: wongedan_kuwibebas] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="wongedan_kuwibebas"
        echo ""
...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- https://raw.githubusercontent.com/messiey/lele/master/ShellBot.sh >/etc/.maAsiss/.Shellbtsss
			}
			[[ "$(grep -wc "Wong_Edan_bot" "/etc/rc.local")" = '0' ]] && {
			    sed -i '$ i\screen -dmS Wong_Edan_bot bbt' /etc/rc.local >/dev/null 2>&1
			}
        }
        screen -dmS Wong_Edan_bot bbt >/dev/null 2>&1
        fun_bot1
        [[ $(ps x | grep "Wong_Edan_bot" | grep -v grep | wc -l) != '0' ]] && echo -e "\nBot successfully activated !" || echo -e "\nError1! Information not valid !"
        sleep 2
        menu
        } || {
       clear
        echo -e "Info...\n"
        fun_bot2() {
            screen -r -S "Wong_Edan_bot" -X quit >/dev/null 2>&1
            [[ $(grep -wc "Wong_Edan_bot" /etc/rc.local) != '0' ]] && {
                sed -i '/Wong_Edan_bot/d' /etc/rc.local
            }
