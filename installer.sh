...
EOF
        ...
        fun_bot1() {
            [[ ! -e "/etc/.maAsiss/.Shellbtsss" ]] && {
				wget -qO- https://raw.githubusercontent.com/jokowaww/bot_panel/main/ShellBot.sh >/etc/.maAsiss/.Shellbtsss
			}
			[[ "$(grep -wc "Wekku_Wekmu_bot" "/etc/rc.local")" = '0' ]] && {
			    sed -i '$ i\screen -dmS Wekku_Wekmu_bot bbt' /etc/rc.local >/dev/null 2>&1
			}
        }
        screen -dmS givpn_bot bbt >/dev/null 2>&1
        fun_bot1
        [[ $(ps x | grep "Wekku_Wekmu_bot" | grep -v grep | wc -l) != '0' ]] && echo -e "\nBot successfully activated !" || echo -e "\nError1! Information not valid !"
        sleep 2
        menu
        } || {
       clear
        echo -e "Info...\n"
        fun_bot2() {
            screen -r -S "Wekku_Wekmu_bot" -X quit >/dev/null 2>&1
            [[ $(grep -wc "Wekku_Wekmu_bot" /etc/rc.local) != '0' ]] && {
                sed -i '/Wekku_Wekmu_bot/d' /etc/rc.local
            }
            rm -f /etc/.maAsiss/bot.conf
            sleep 1
        }
        fun_bot2
        echo -e "\nBot Wekku_Wekmu_bot Stopped!"
        sleep 2
        menu
    }
}
